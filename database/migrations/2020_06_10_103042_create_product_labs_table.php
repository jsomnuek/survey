...
class CreateProductLabsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('product_labs', function (Blueprint $table) {
            ...
            $table->integer('testing_calibrating_type_id')->comment('ประะเภทการทดสอบ');//4.6ประเภทการทดสอบ/สอบเทียบ
            ...
            $table->string('product_lab_test_control_other', 255)->comment('อื่นๆการควบคุมคุณภาพผลการทดสอบ');//4.14การควบคุมคุณภาพผลการทดสอบ 
        ...
        Schema::create('product_lab_calibrating', function (Blueprint $table) {
            $table->bigIncrements('id');
            $table->unsignedBigInteger('product_lab_id');
            $table->unsignedBigInteger('testing_calibrating_id');
            $table->timestamps();
            
            $table->foreign('product_lab_id')
                ->references('id')->on('product_labs');
            $table->foreign('testing_calibrating_id')
                ->references('id')->on('testing_calibrating_types');
        });
    }
    ...
    public function down()
    {
        ...
        Schema::dropIfExists('product_lab_calibrating');
    }
}
